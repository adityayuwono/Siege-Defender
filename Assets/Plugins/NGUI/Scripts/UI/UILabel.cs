...
public class UILabel : UIWidget
{
    ...
    [HideInInspector]
    [SerializeField]
    NGUIText.SymbolStyle mSymbols = NGUIText.SymbolStyle.Uncolored;
    [HideInInspector]
    [SerializeField]
    ...
    public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
    {
        ...
        NGUIText.tint = col;
        NGUIText.Print(text, verts, uvs, cols, mFontSize);
    ...
    #region contenga
    public int GetCharIndexAtPos(Vector2 worldPos)
    {
        Vector2 temp = this.transform.parent.worldToLocalMatrix.MultiplyPoint3x4(worldPos);
        return GetCharIndexAtPos(this, text, temp, new Vector2(transform.localPosition.x, transform.localPosition.y), this.transform.localScale.x);
    }

    public Vector2 GetCharPosAtIndex(int idx)
    {
        Vector2 vec = new Vector2(0, 0);
        vec = GetCharPosAtIndex(this, text, idx, this);
        vec.x *= this.transform.localScale.x;
        vec.y *= this.transform.localScale.y;
        vec.x += this.transform.localPosition.x;
        vec.y += this.transform.localPosition.y;
        return vec;
    }

    public int GetCharIndexAbove(int idx)
    {
        return GetCharIndexAbove(this, text, idx);
    }

    public int GetCharIndexBelow(int idx)
    {
        return GetCharIndexBelow(this, text, idx);
    }

    private static Vector2 GetCharPosAtIndex(UILabel uiLabel, string text, int idx)
    {
        return GetCharPosAtIndex(uiLabel, text, idx, null);
    }

    private static Vector2 GetCharPosAtIndex(UILabel thisLabel, string text, int idx, UILabel uiLabel)
    {
        var mFont = thisLabel.bitmapFont.bmFont;
        const int M_SPACING_X = 0;
        const int M_SPACING_Y = 0;

        Vector2 v = Vector2.zero;
        if (mFont != null && mFont.isValid/* && !string.IsNullOrEmpty(text)*/)
        {
            /*if (encoding) */
            List<int> subAndSuperTagIndexList;// List of all occurences of subcript and superscript

            text = NGUIText.StripSymbols(text, out subAndSuperTagIndexList);

            float x = 0; float y = 0;
            int prev = 0;
            int lineHeight = (mFont.charSize + M_SPACING_Y);
            float lengthSinceLastSpace = 0;// Adjustment for auto Line break
            bool isLabelValid = (uiLabel != null && uiLabel.width != 0);
            ...
            for (int i = 0, imax = text.Length; i < imax; ++i)
            {
                ...
                char newChar = text[i];
                BMGlyph glyph = mFont.GetGlyph(newChar);
                float charLength = 0;

                // Calculate character length using NGUI's glyph
                if (glyph != null)
                {
                    charLength = M_SPACING_X + ((prev != 0) ? glyph.advance + glyph.GetKerning(prev) : glyph.advance);
                    charLength *= (subAndSuperTagIndexList.Contains(i) ? 0.75f : 1f);// Adjust for Sub and Super modified Length
                }

                if (glyph != null)
                {
                    x += charLength;
                    if (newChar != ' ' || !(lengthSinceLastSpace < float.Epsilon))
                    {
                        lengthSinceLastSpace += charLength;
                    }
                }

                // Start a new line
                if (newChar == '\n' || (isLabelValid && x >= uiLabel.width))
                {
                    x = (newChar == '\n') ? 0 : lengthSinceLastSpace;
                    y += lineHeight;
                    prev = 0;
                    continue;
                }

                if (text[i] == ' ' && isLabelValid && x + charLength < uiLabel.width)
                {
                    lengthSinceLastSpace = 0;
                }
    ...
    private static int GetCharIndexAtPos(UILabel uiLabel, string text, Vector2 pos, Vector2 offset, float scale)
    {
        var mFont = uiLabel.bitmapFont.bmFont;
        const int M_SPACING_X = 0;
        const int M_SPACING_Y = 0;
        ...
        if (mFont != null && mFont.isValid && !string.IsNullOrEmpty(text))
        {
            ...
            for (int i = 0, imax = text.Length; i <= imax; ++i)
            {
                char c = i < imax ? text[i] : 'w';

                // Skip invalid characters
                if (c < ' ' && c != '\n') { prev = 0; continue; }

                var glyph = mFont.GetGlyph(c != '\n' ? c : 'w');
                if (glyph == null)
                    throw new NullReferenceException(string.Format("Glyph for character '{0}' could not be found.", c));

                x += M_SPACING_X + ((prev != 0) ? glyph.advance + glyph.GetKerning(prev) : glyph.advance);
                prev = c;

                var scalee = (mFont.charSize > 0) ? 1f / mFont.charSize : 1f;
                var tempX = scalee * scale * x + offset.x;
                var tempY = scalee * scale * y + offset.y;
                var tempWidth = scalee * scale * glyph.advance / 2;
                var tempHeight = scalee * scale * lineHeight / 2;

                //float dist = Mathf.Abs(pos.x - (tempX - tempWidth)) + Mathf.Abs(pos.y - (tempY - tempHeight));
                float distX = Mathf.Abs(pos.x - (tempX - tempWidth));
                float distY = Mathf.Abs(pos.y - (tempY - tempHeight));

                if (distY <= minDistY)
                {
                    if (distY < minDistY)
                        minDistX = float.PositiveInfinity;
    ...
    private static int GetCharIndexAbove(UILabel uiLabel, string text, int idx)
    {
        var mFont = uiLabel.bitmapFont.bmFont;
        var mSpacingY = uiLabel.spacingY;

        float scale = (mFont.charSize > 0) ? 1f / mFont.charSize : 1f;
        Vector2 v = GetCharPosAtIndex(uiLabel, text, idx);

        int lineHeight = (mFont.charSize + mSpacingY);

        v.y += scale * lineHeight * 1.5f;
        return GetCharIndexAtPos(uiLabel, text, v, Vector2.zero, 1);

    }

    private static int GetCharIndexBelow(UILabel uiLabel, string text, int idx)
    {
        var mFont = uiLabel.bitmapFont.bmFont;
        var mSpacingY = uiLabel.spacingY;
