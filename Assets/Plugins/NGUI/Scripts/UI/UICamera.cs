...
/// - OnSelect2 is Contenga added
/// - OnSelect2 ([selected, raycastHit]) is the same as OnSelect, with raycast hit info to detect click position and still sent even when the object already selected
...
public class UICamera : MonoBehaviour
{
    ...
    {
        public int index;               // Contenga: Mouse Index to tell between left and right clicks
        ...
        // Contenga - to countinue the raycast
        public GameObject nextCurrent;	// The current game object under the touch or mouse
    ...
    public float tooltipDelay = 0.3f; //Contenga 1f; old value
        ...
        else if (mCurrentSelection != go)
        {
            if (mCurrentSelection != null)
            {
                ...
                if (uicam != null)
                {
                    ...
                    //Contenga
                    Notify(mCurrentSelection, "OnSelect2", new object[] { false, null });
                    current = null;
                }
            }
    ...
    System.Collections.IEnumerator ChangeSelection()
    {
        ...
        if (mCurrentSelection != null)
        {
            ...
            Notify(mCurrentSelection, "OnSelect2", new object[] { true, null });
            current = null;
        }
    }
 ...
 || Application.platform == RuntimePlatform.BB10Player
    ...
    private float _lastDistanceToPlayer = float.PositiveInfinity;
    #endregion
    /// <summary>
    /// Update mouse input.
    /// </summary>
    ...
    {
        #region Contenga Alternate Input
        // Update the position and delta
        lastTouchPosition = GetLastInputPosition();
        #endregion
        ...
        if (isPressed || posChanged || mNextRaycast < RealTime.time)
        {
            ...
            if (!Raycast(lastTouchPosition, out lastHit)) hoveredObject = fallThrough;
            if (hoveredObject == null) hoveredObject = genericEventHandler;
            for (int i = 0; i < 3; ++i) mMouse[i].current = hoveredObject;
        }


        var wasTargetWithinReach = _lastDistanceToPlayer < ClickDistance;
        var distanceToPlayer = mMouse[0].current != null ? DistanceToPlayer(mMouse[0].current) : float.PositiveInfinity;
        _lastDistanceToPlayer = distanceToPlayer;
        var isTargetWithinReach = distanceToPlayer < ClickDistance;
        ...
        // (Rudy) Also broadcast if the target is beyond reach
        if ((justPressed || !isPressed) && mHover != null && (highlightChanged || (!isTargetWithinReach && wasTargetWithinReach)))
        {
            currentScheme = ControlScheme.Mouse;
        ...
        for (int i = 0; i < 3; ++i)
        {
            bool pressed = GetMouseButtonDown(i);
            bool unpressed = GetMouseButtonUp(i);
        ...
        // (Rudy) Only broadcast if the target is within reach
        if (!isPressed && (isTargetWithinReach && (highlightChanged || !wasTargetWithinReach)))
        {
            currentScheme = ControlScheme.Mouse;
            mTooltipTime = RealTime.time + tooltipDelay;
        ...
        if (unpressed)
        {
            ...
            if (currentTouch.pressed != null)
            {
                    ...
                    currentTouch.totalDelta.magnitude < drag))
                {
                    ...
                    #region Contenga Send OnClick with Hand
                    // Specific Hand click
                    // Because we are using Hand to grab the objects, we need to differentiate between left and right mouse clicks
                    // Just for reminder: This is heavily changed by Contenga(Adit), because we have a special requirement that involves a "Hand" to grab objects
                    if (currentTouch.clickNotification != ClickNotification.None)
                    {
                        SendOnClickWithHand(mHover);
                    }
                    #endregion
                }
    ...
    #region Contenga Overrideable Methods

    protected virtual Vector2 GetLastInputPosition()
    {
        return lastTouchPosition;
    }

    protected virtual bool IsTargetWithinReach(GameObject mHover)
    {
        return true;
    }

    protected virtual float ClickDistance
    {
        get { return float.PositiveInfinity; }
    }

    protected virtual float DistanceToPlayer(GameObject target)
    {
        return float.PositiveInfinity;
    }

    protected virtual bool GetMouseButtonDown(int i)
    {
        return Input.GetMouseButtonDown(i);
    }

    protected virtual bool GetMouseButtonUp(int i)
    {
        return Input.GetMouseButtonUp(i);
    }

    protected virtual void CleanupPressTrigger()
    { }

    protected virtual void SendOnClickWithHand(GameObject lastHoveredGameObject)
    { }
